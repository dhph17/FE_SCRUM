    ...
    return (
        <Parent>
            <Panel role="parent" activeItem={3}>
                <p className="font-semibold text-[1.2rem] text-shadow-sm mb-4">Danh sách gia sư đăng kí</p>
                <div className="border-[3px] rounded-[1rem] border-[#002182] shadow-md bg-white py-4 flex flex-col">
                    ...
                    <div className="px-4 mt-3 flex flex-col self-center mb-3 w-[70%]">
                        <p className="text-[1.2rem] font-bold text-custom_darkblue ">Danh sách gia sư đăng kí: </p>
                        <div className="mt-4 max-h-[15rem] px-3 overflow-auto py-4 rounded-xl scrollbar scrollbar-thumb-white/30 shadow-md bg-slate-100 border border-slate-200 ">
                            {
                                listSong.map((song, index) => (
                                    <div key={index} className="flex justify-between items-center px-5 py-1 cursor-pointer mb-1 hover:bg-slate-200 rounded-xl">
                                        <div className="flex items-center gap-3">
                                            <img src={song.poster} alt="" className="w-[50px] h-[50px] rounded-full object-cover" />
                                            <p className="font-semibold text-[1.1rem]">{song.artist}</p>
                                        </div>
                                        <div className="flex gap-6">
                                            <button className="font-semibold text-[0.9rem] h-[2rem] w-[8rem] bg-custom_yellow rounded-md">Xem thông tin</button>
                                            <button className="font-semibold text-[0.9rem] h-[2rem] w-[8rem] bg-custom_darkblue text-white rounded-md">Nhận gia sư</button>
                                        </div>
                                    </div>
