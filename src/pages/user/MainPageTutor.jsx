    ...
    <Tutor>
      <Panel_Search className="flex flex-col max-h-full">
        <div className='flex items-center mb-2 -mt-2'>
          ...
          <p className='font-semibold text-[1.2rem] text-shadow-sm'>BÀI ĐĂNG GẦN ĐÂY</p>
        </div>
        <div className="relative max-h-[38rem] overflow-y-auto grid grid-cols-1 gap-4">
          {currentPosts.map((tutor, index) => (
            ...
            >
              <ItemPost user={tutor} >
                <button className="bg-yellow-500 w-[14vw] p-2 rounded-2xl font-semibold mx-8">Đăng kí dạy</button>
              </ItemPost>
            </div>
          ))}
        </div>
